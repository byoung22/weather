...
export class Data {
  ...
  async addData(city) {
    try {
      ...
      const response = await fetch(
        `https://api.weatherapi.com/v1/forecast.json?key=a91fd93998594df68e9202550232006&q=${city}&days=3&aqi=yes`
      );
      ...
      let sunriseTime = parseInt(
        data.forecast.forecastday[0].astro.sunrise.split(":")[0],
        10
      ); // Outputs hour of sunrise
      let sunsetTime =
        parseInt(data.forecast.forecastday[0].astro.sunset.split(":")[0], 10) +
        12; // Outputs hour of sunset
      // If time passed, use tomorrow's sunset and sunrise
      if (currentTime > sunriseTime)
        sunriseTime = parseInt(
          data.forecast.forecastday[1].astro.sunrise.split(":")[0],
          10
        );
      if (currentTime > sunsetTime)
        sunsetTime =
          parseInt(
            data.forecast.forecastday[1].astro.sunset.split(":")[0],
            10
          ) + 12;

      for (let i = 0; i < 27; i++) {
        if (currentTime < 24) {
          // Store remaining hours of the day
          this.hours[i] = {};
          this.hours[i].hour = new Date(
            data.forecast.forecastday[0].hour[currentTime].time
          ).getHours();
          this.hours[i].condition =
            data.forecast.forecastday[0].hour[currentTime].condition.text;
          if (this.temp === "F")
            this.hours[i].temp =
              data.forecast.forecastday[0].hour[currentTime].temp_f;
          if (this.temp === "C")
            this.hours[i].temp =
              data.forecast.forecastday[0].hour[currentTime].temp_c;
        } else {
          // Store the rest of 24 hour period
          this.hours[i] = {};
          this.hours[i].hour = new Date(
            data.forecast.forecastday[1].hour[currentTime - 24].time
          ).getHours();
          this.hours[i].condition =
            data.forecast.forecastday[1].hour[currentTime - 24].condition.text;
          if (this.temp === "F")
            this.hours[i].temp =
              data.forecast.forecastday[1].hour[currentTime - 24].temp_f;
          if (this.temp === "C")
            this.hours[i].temp =
              data.forecast.forecastday[1].hour[currentTime - 24].temp_c;
        }
  ...
  addDay() {
    ...
    this.forecast[1].day =
      weekday[d.getDay() + 1 > 6 ? d.getDay() - 6 : d.getDay() + 1];
    this.forecast[2].day =
      weekday[d.getDay() + 2 > 6 ? d.getDay() - 5 : d.getDay() + 2];
  }
